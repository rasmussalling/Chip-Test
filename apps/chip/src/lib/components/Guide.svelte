...
<article 
  class="prose prose-invert w-[800px] h-[1000px] max-w-full outline-none flex flex-col overflow-hidden mx-auto"
  tabindex="-1" 
  bind:this={container}
>
  <header class="mb-6 flex items-start justify-between gap-4 flex-shrink-0">
    <div>
  ...
  <footer class="mt-auto flex flex-col gap-6 pt-8 flex-shrink-0 bg-inherit">
    ...
    <div class="flex items-center justify-between border-t border-slate-700/50 pt-6 pb-2">
      ...
      <select
        class="ml-4 rounded bg-slate-900/60 px-3 py-1 text-lg transition hover:bg-slate-900 outline-none focus:ring-2 focus:ring-blue-500"
        bind:value={page}
      >
        {#each pages as p, i}
